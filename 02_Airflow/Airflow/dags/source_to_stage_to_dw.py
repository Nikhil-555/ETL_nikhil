...
default_args = {
    ...
    'start_date': datetime(2024, 12, 3),  # Adjust as needed
}
...
with DAG(
    ...
    description='ETL DAG combining Oracle to S3, S3 to Redshift devstage, and Redshift devstage to devdw',
    ...
    # Define TaskGroup: S3 to Redshift devstage
    ...
    # Define TaskGroup: Redshift devstage to devdw
    with TaskGroup(group_id='redshift_devstage_to_devdw') as devstage_to_devdw:
        start_devstage_to_devdw = DummyOperator(task_id='start_devstage_to_devdw')
        end_devstage_to_devdw = DummyOperator(task_id='end_devstage_to_devdw')

        # Define Bash tasks for devstage to devdw scripts
        scripts_folder = "/mnt/c/Nikhil/ETL_PYTHON/ETL_nikhil/01_python/stage_to_dw_scripts"
        scripts = [
            'offices.py',
            'employees.py',
            'customers.py',
        ...
        # Set TaskGroup dependencies
        start_devstage_to_devdw >> tasks >> end_devstage_to_devdw

    # Define overall DAG dependencies
    start_dag >> oracle_to_s3 >> s3_to_redshift >> devstage_to_devdw >> end_dag
